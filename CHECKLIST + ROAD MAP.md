...
| Сохранение состояния | Геометрия окна, ширина столбцов и порядок сортировки сохраняются в `QSettings`. | После повторного открытия таблица восстанавливает ранее выбранные параметры. | работает | 2025-09-23 |
...
| Элемент | Статичное состояние | Поведение при взаимодействии | Статус | Дата |
| --- | --- | --- | --- | --- |
| Таблица расписания | Рамки угловатые, неон почти не заметен. | Клик подсвечивает всю таблицу розовым контуром; количество строк теперь подстраивается под длину месяца, а ввод дней ограничен валидным диапазоном. | требует доработки | 2025-09-21 |
...
| Таблица результатов | Неон не отображается, выделение розовым контуром. | Ширина колонок и выбранная сортировка сохраняются между сессиями. | работает | 2025-09-23 |
...
| Элемент | Статичное состояние | Поведение при взаимодействии | Статус | Дата |
| --- | --- | --- | --- | --- |
| Версия приложения | Файл `VERSION` содержит `3.1.6`, обновление номера выполняется вручную. | Требуется повышать версию при каждом фиксировании изменений. | требует доработки | 2025-09-23 |
