...
| Метка версии | QLabel `v<номер>`, Exo 2 Regular ~9 pt, цвет `#202020`; файл `VERSION` содержит `1.2.4` вместо требуемого `3.1.2`. | Обновление версии не автоматизировано, фиксы не повышают номер. | требует доработки | 2025-09-16 |
...
| Сохранение состояния | Параметры окна и ширины колонок не запоминаются. | После закрытия настройки сбрасываются. | требует доработки | 2025-09-16 |
...
| Элемент | Статичное состояние | Поведение при взаимодействии | Статус | Дата |
| --- | --- | --- | --- | --- |
| Версия приложения | Файл `VERSION` содержит `1.2.4`, должна быть `3.1.2`; обновление номера не автоматизировано. | Требуется повышать версию при каждом фиксировании изменений. | требует доработки | 2025-09-16 |
...
7. **Версионирование и статус-бар.**
   - Автоматизировать обновление `VERSION`: скрипт после каждого merge повышает номер патча, переход на `3.1.2` фиксируется в changelog; статус-бар считывает номер динамически.
