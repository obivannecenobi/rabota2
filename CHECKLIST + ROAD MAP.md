...
| Кнопка «Свернуть панель» | Градиент `#39ff14→#2d7cdb`, радиус 16 px, иконка 28×28 `#ffffff`, подпись «Свернуть» центрируется как у соседних пунктов; ширина кнопки расширяется вместе с панелью. | Hover/клик повторяют `accent_color`, неон остаётся активным, при сворачивании кнопка переходит в icon-only и сохраняет подсветку. 2025-10-31: повторное применение темы возвращает текст и отступы без рассинхронизации. | частично решено | 2025-10-31 |
...
| Кнопка «Настройки» | StyledToolButton с текстом «Настройки», градиент `#39ff14→#2d7cdb`, радиус 16 px; в развёрнутом режиме текст и иконка выравниваются как у остальных пунктов, при сворачивании остаётся только иконка. 2025-10-30: тест подтверждает сохранение подписи после повторного раскрытия. | Hover/активное состояние повторяет актуальный `accent_color`, неон остаётся активным при переключении ширины панели. 2025-10-30: ButtonStyleMixin применяет профиль idle/hover. | частично решено | 2025-10-30 |
...
| Кнопки «Предыдущий/Следующий» | Градиент `#39ff14→#2d7cdb`, радиус 16 px, стрелки 22×22 автоматически тонируются в актуальный `accent_color`, кешируются для размеров. | Hover: фон становится `#3a3a3a`, неоновая подсветка сохраняет актуальный акцент, иконки обновляются при смене темы. | частично решено | 2025-10-29 |
| Метка месяца | QLabel Exo 2 Bold 24 px, базовый стиль хранится без цвета, оттенок текста берётся из актуального `accent_color` с учётом монохромного режима, неон активен. | При смене темы helper пересобирает палитру, стиль и интенсивность свечения, цвет синхронизируется с `CONFIG["accent_color"]`. | решено | 2025-10-19 |
...
| Фон | Корпус диалога повторяет `workspace_color`, радиус 16 px, рамка подхватывает `accent_color`, мягкое свечение включено. | При смене темы фон и неон пересобираются, стиль обновляется без перезапуска окна. | решено | 2025-11-01 |
...
| Таблица результатов | NeonTableWidget синхронизирован с `workspace_color`, рамка и текст подхватывают `accent_color`; 2025-10-06: `TopDialog.refresh_theme()` пересобирает стиль при открытии. 2025-10-24: заголовок столбца исправлен на «Запланировано». | Hover: фильтр поддерживает неон и текущий акцент, после смены темы окно обновляет подсветку без розового контура. | частично решено | 2025-10-24 |
...
| Шрифтовая тема | Exo 2 остаётся предустановленной гарнитурой; списки шрифтов фильтруют варианты без полного набора латиницы и кириллицы, неподдерживаемые значения откатываются к Exo 2. | Селекторы показывают только валидные гарнитуры, попытка выбрать неподдерживаемый шрифт откатывается и сопровождается предупреждением. | решено | 2025-10-28 |
...
1. **Типографика и поддержка графических шрифтов.**
   - Зафиксировать Exo 2 как предустановленный шрифт для всех текстовых элементов, добавив проверку конфигурации (`header_font`, `sidebar_font`, `workspace_font`) при запуске. 2025-10-28: SettingsDialog фильтрует список гарнитур по наличию латиницы/кириллицы и откатывает конфиг к Exo 2 при выборе неподдерживаемого значения.
