...
| Кнопка «Топы» | StyledToolButton, иконка `ICON_TP`, текст белый, градиент и радиус 16 px. | Hover и активное состояние повторяют поведение «Выкладка»; кликом открывается диалог TopDialog, после закрытия подсветка остаётся на выбранной кнопке даже при ручном сворачивании панели. | работает | 2025-09-17 |
